...
function step2(age, EA, AdjD){
    ...
    if(age>=13){
        ...
        if(7<=EA){
            if(AdjD<0){
                textData.push(`[잠정 결과3] 일반적 통제능력(Adj D)과 가용 자원(EA)을 통해 볼 때, 수검자의 통제능력과 스트레스 저항력을 반영하는 지표가 타당한지에 대한 판단을 보류해야 한다.`);
        ...
        if(11<EA){
            if(AdjD>0){
                textData.push(`[잠정 결과4] 일반적 통제능력(Adj D)과 가용 자원(EA)을 통해 볼 때, 수검자의 통제능력과 스트레스 저항력을 반영하는 지표는 타당하고 신뢰할 수 있다.`);
                result.nextStep = 3;
                result.textData = textData;
                result.curStep = 2;
                return result;
            ...
            if(AdjD==0){
                textData.push(`[잠정 결과5] 일반적 통제능력(Adj D)과 가용 자원(EA) 점수를 통해 볼 때, 수검자의 통제능력과 스트레스 저항력을 반영하는 지표는 수검자의 원래 능력 수준보다 낮게 추정되었을 가능성이 있다.`);
        ...
        if(EA<7){
            if(AdjD<0){
                textData.push(`[잠정 결과6a] 일반적 통제능력(Adj D)과 가용 자원(EA)을 통해 볼 때, 수검자의 가용 자원이 정상집단의 예상값보다 더 제한되어 있기에 통제능력과 스트레스 저항력을 반영한 지표가 음수를 보이는 것은 쉽게 예상할 수 있다. 현재 수검자의 일반적 통제능력(Adj D) 점수는 타당하고 신뢰할 수 있다.`);
            ...
            if(AdjD>=0){
                textData.push(`[잠정 결과6b] 일반적 통제능력(Adj D)과 가용 자원(EA)을 통해 볼 때, 수검자의 가용 자원이 정상집단의 예상값보다 더 제한되어 있지만, 수검자는 상황 단서가 상당히 명료하고 잘 구조화되어 있는 환경에서는 더 효과적으로(effectively) 기능할 수 있다. `);
    ...
    if(age<10){
        ...
        if(EA<4){
            ...
            if(AdjD>=0){
                textData.push(`[잠정 결과 6b] 일반적 통제능력(Adj D)과 가용 자원(EA)을 통해 볼 때, 수검자의 가용 자원이 정상집단의 예상값보다 더 제한되어 있지만, 수검자는 상황 단서가 상당히 명료하고 잘 구조화되어 있는 환경에서는 더 효과적으로(effectively) 기능할 수 있다. `);
                textData.push(`가용 자원이 낮아(Low EA) 복잡한 사회를 살다 보면 겪게 되는 다양한 평소의 일상적인 스트레스에도 쉽게 심리가 혼란될(disorganized) 정도로 만성적으로 더 취약한 수검자로 잘못 해석할 수 있으니 주의해야 한다. `);
                textData.push(`어린 아동의 경우 심리적 발달이 충분하지 않아 가용 자원(EA)이 예상한 수준만큼 나타나지 않을 수 있으므로 해석에 주의해야 한다.`);
                result.nextStep = 3;
                result.textData = textData;
...
function step3(EBLeft, EBRight, EA, Lambda, age, M, WSumC){
    ...
    if(EBLeft>0 && EBRight>0){
        if((EA>3.5 && Lambda<=0.99) || (EA>6 && Lambda>=1)){
            textData.push(`[잠정 결과1] [정상범주] 가용 자원비율(EB)과 Lambda(L)를 통해 볼 때, 가용 자원(EA)의 지표와 일반적 통제능력(Adj D)의 지표는 타당하고 신뢰할 수 있다.`);
            result.nextStep = 4;
            result.textData = textData;
            result.curStep = 3;
            result.goNext = false;
        ...
        if(EA<4){
            if(Lambda<=0.99){
                textData.push(`[잠정 결과2] 가용 자원비율(EB)과 Lambda(L)를 통해 볼 때, 가용 자원(EA)의 지표는 신뢰할 수 있으나, 일반적 통제능력(Adj D)의 지표는 신뢰할 수 없다.`);
            ...
            if(Lambda>=1){
                textData.push(`[잠정 결과4] 가용 자원비율(EB)과 Lambda(L)를 통해 볼 때, 가용 자원(EA)의 지표는 타당하지 않다. 그러므로 가용 자원(EA)을 토대로 산출하는 통제능력(D, Adj D) 지표를 통해 수검자의 통제능력을 평가하는 것은 적절하지 않다.`);
        ...
        if((age>=18 && (3.5<EA && EA<6.5)) || ((13<=age && age<=17) && (3.5<EA && EA<7))){
            if(Lambda>=1){
                textData.push(`[잠정 결과3] 가용 자원비율(EB)과 Lambda(L)를 통해 볼 때, 가용 자원(EA)의 지표는 타당하며, 일반적 통제능력(Adj D) 지표는 타당하지 않을 수 있다.`);
    ...
    if(EBLeft==0 || EBRight==0){
        if(M==0 && WSumC>3.5){
            textData.push(`[잠정 결과5] 가용 자원비율(EB)과 Lambda(L)를 통해 볼 때, 가용 자원(EA)의 지표와 일반적 통제능력(Adj D) 지표를 통해 유추한 수검자의 통제능력에 대한 가설은 적절하지 않다.`);
        ...
        if(M>3 && WSumC==0){
            textData.push(`[잠정 결과6] 가용 자원비율(EB)과 Lambda(L)를 통해 볼 때, 가용 자원(EA)의 지표와 일반적 통제능력(Adj D) 지표를 통해 유추한 수검자의 통제능력에 대한 가설은 적절하지 않다. `);
            textData.push(`수검자의 일반적 통제능력(Adj D)은 파악할 수 없으며, 현재 통제능력(D)은 취약한 상태라고 결론 내리는 게 적절하다. 주의할 점은 현재 통제능력(D) 지표가 해석적 의미가 있으려면, 가용 자원(EA)은 5.0을 초과하고, 회피 유형에 해당하지 않아야(Lambda(L) < 1.0) 한다는 것이다.`);
...
function step5(es, FM, m, SumCprime, SumV, SumT, SumY){
    ...
    if(es>=4 && (FM+m<SumCprime+SumV+SumT+SumY)){
        ...
        textData.push(`일반적으로 정상집단의 경우 eb의 좌항(FM+m)이 우항(SumC'+SumV+SumT+SumY)보다 클 것이므로 수검자가 보이는 이러한 점수 형태는 일반적인 양상이 아닐 수 있다.</br>`);
    }
    if(FM>=6){
        ...
        textData.push(`욕구 관련 요구(need-related demand)는 의도적 사고방식을 방해하고, 주의력과 집중력에 간섭할 수 있다. 다시 말해서, 의식적 사고 활동이 적절하게 이행되지 않고, 주의력과 집중력이 저하 될 수 있다. 이러한 종류의 통제 밖에 있는 외곽사고는 강박사고, 외상 경험, 정서 결핍 때문에 나타날 수 있다.</br>`);
    }
    if(FM<=1){
        ...
        textData.push(`또는 다른 사람들보다 빠르게 행동화하여 해소되는 것을 의미할 수 있다. 이러한 경향성은 통제능력이 적절하게 기능하지 않는 충동적인 모습이나, 억제되지 않는 자극추구 경향으로 보일 수 있다.</br>`);
    }
    if(SumCprime>=3){
        ...
        textData.push(`이러한 심리 과정은 불안, 슬픔, 긴장, 걱정 등을 포함하는 주관적 불편을 경험하게 한다. 그리고 신체적 기능 이상의 원인이 될 수도 있다.</br>`);
    }
    if(SumV>=1){
        ...
        textData.push(`
        * 차원(Vista) 반응을 조정한 이유에 대해서도 다룬다.</br>`);
    }
    if(SumT>=2){
        ...
        * 재질(Texture) 반응을 조정한 이유에 대해서도 다룬다.
        `);
        textData.push(`개인력에서 최근의 상황적 요인이 발견되지 않는다면, 정서적 결핍이나 고독감의 근원이 오래되었으며, 대인관계에서 과도한 친밀감의 욕구를 보인다고 가정하는 게 적절하다.</br>`);
    }
    result.textData = textData;
    result.curStep = 5;
    result.nextStep = 6;
